...
let MATRIZ: number[] = new Array(FIM).fill(0);
let INICIO: number = 0;
...
function Adicionar(ELEMENTO: number): boolean {
  if (Cheia())
    return false;
  else {
...
function Saida() {
  ...
  if (ELEMENTO != null)
    console.log(`Elemento ${ELEMENTO} retirado do inicio da fi-la.`);
  else
